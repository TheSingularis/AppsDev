...
switch ($controllerChoice) {                //naming convention is "'location'_'action'" i.e. todo_list or task_list
    case 'todo_list':
        ...
        if (isset($_SESSION['userId'])) {   //set on login
            $userId = $_SESSION['userId'];
        }

        $todos = ($user->getUserTypeId() == 1) ? ToDoDB::getAllToDos() : ToDoDB::getAllToDosByUserId($userId);

        include 'todo_list.php';
        break;
    case 'todo_list_search':
        $email = filter_input(INPUT_POST, 'searchValue');

        $todos = ToDoDB::getToDosByEmail($email);
    ...
    case 'task_delete':
        ...
        include 'task_delete.php';
        break;
    case 'task_delete_process':
        $taskId = filter_input(INPUT_POST, 'taskId');
